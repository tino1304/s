...
4. **Spawn ALL subtasks in a SINGLE message** using Task tool (parallel). Use "general-purpose" for code, "Explore" for research. Each agent prompt MUST start with: "First, read `.claude/s/skills/mapper.md` and load the relevant skill files from `.claude/s/skills/` before writing any code." followed by the subtask details.
